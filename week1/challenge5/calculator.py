...
class Args(object):
    def __init__(self):
        self.conf, self.userdata, self.resfile, self.city = self._parser_args()
    ...
    def _parser_args(self):
        ...
        for opt,arg in opts:
            if opt == '-c':
                ...
                resfile = arg
        return conf,userdata,resfile,city

class Config(object):
    def __init__(self,city,conf):
        self.JiShuH, self.JiShuL, self.scioal_insurance_rate = self._parse_config(city,conf)

    def _parse_config(self,city,conf):
        config = ConfigParser()
        config.read(conf)
        scioal_insurance_rate = 0
        for para in config[city]:
            val = config[city][para]
            if para == 'jishuh':
                JiShuH = float(val)
            elif para == 'jishul':
                JiShuL = float(val)
            else:
                scioal_insurance_rate += float(val)
...
class Calculator(object):
    ...
    def calc_all(self,config,salay):
        ...
        t = datetime.now()
        outtime = datetime.strftime(t,'%Y-%m-%d %H:%M:%S')
        return self.number,self.salay,insurance_money,tax,real_income,outtime
...
class Execute(object):
    def __init__(self):
        a = Args()
        self.config = Config(a.city,a.conf)
